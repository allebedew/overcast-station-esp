 ...
 * (datasheet: 2 ms) and a reset leaves it in deep standby, where the access
 * that wakes the I2C block is itself lost. Retrying covers both. Busy-waited:
 * a vTaskDelay() of a few ms rounds down to no delay at all. */
...
/* OSR_CONFIG: osr_t bits 2:0, osr_p bits 5:3, press_en bit 6. x2/x16 is the
 * datasheet's "standard" indoor setting, ~1 Pa RMS noise. */
 ...
 * shdw_sel_iir_p routes the filtered pressure into the data registers; without
 * it the filter runs but nothing sees its output.
 *
 * DSP_IIR: set_iir_t bits 2:0, set_iir_p bits 5:3, coefficient codes
 * 0 = bypass, 1 = 1, 2 = 3, 3 = 7, 4 = 15, ... The filter counts samples, not
 * seconds, so 15 at the 15 Hz ODR below is a time constant near a second.
 * Temperature is unfiltered — it comes from the TMP117 anyway.
 *
 * Both are written in standby; the DSP config is not meant to change in normal
 * mode. */
...
/* ODR_CONFIG: pwr_mode bits 1:0, odr bits 6:2, deep_dis bit 7. Normal mode at
 * 15 Hz (odr code 0x16); deep standby must be off or the sensor leaves normal
 * mode at low ODR.
 *
 * The station polls at 4 Hz. Matching the two rates would let the clocks drift
 * against each other and duplicate or skip samples; ~4 conversions per poll
 * removes the question. 15 Hz also stays under the ~25 Hz the part manages at
 * x16 — beyond that it silently drops the oversampling (OSR_EFF below). */
...
static esp_err_t configure(void)
{
    ...
    /* filter and oversampling first: writing ODR_CONFIG starts the conversions
     * and neither is meant to change once they run */
    s_step = "iir";
